...
function createRenewSHPlugin({ name, aliases=[], title, commandTpl, expMode = 'days' }) {
  ...
  function daysToExpStr(days) {
    ...
    // default: pakai angka hari langsung
    return String(days);
  }
  ...
  async function start(bot, msg) {
    ...
    global.__renewssh_sessions[key] = { step: 1, vpsList };
  ...
  async function cont(bot, msg) {
    ...
    if (S.step === 2) {
      // ✅ FIX REGEX: minus diletakkan di akhir agar tidak dianggap range
      ...
      // tampilkan daftar harga agar user tahu opsi valid
      const listHarga = hargaListText();
      await send(
        bot,
        msg.chat.id,
        `⏳ Masukkan *lama hari* aktif (contoh: \`30\`).\n\n*Daftar Harga:*\n${listHarga}\n\n*Cek Lagi Username* yang sudah anda masukkan.\n\n Ketik /menu jika salah ketik username agar proses tidak dilanjutkan dan tidak mengurangi saldo anda`
      );
      return true;
    }
    ...
    if (S.step === 3) {
      ...
      try {
        ...
        // contoh: "Renew VMess" -> "Renew-vmess"
        const kind = (title || 'renew-ssh').toLowerCase().replace(/\s+/g, '-');
        logPurchase({
          tg_id : msg.from.id,
          kind  : kind,      // renew-ssh | renew-vmess | renew-vless | renew-trojan (sesuai title)
          days  : S.days,    // durasi yang dipilih
          vps_id: vpsLabel
        });
      } catch (e) {
  ...
  return {
    name,
    aliases,
    description: `${title} (pakai saldo; harga dari harga.json; tanpa password; output asli, ANSI dihapus)`,
