...
// ===== validasi username Trojan/Xray =====
function validateTrojanUser(username) {
  try {
...
// ===== plugin creator =====
function createRenewTRPlugin({ name, aliases=[], title, commandTpl, expMode = 'days' }) {
  global.__renewtr_sessions ??= Object.create(null);
  ...
  async function start(bot, msg) {
    ...
    global.__renewtr_sessions[key] = { step: 1, vpsList };

    setTimeout(() => {
      const S = global.__renewtr_sessions[key];
      if (S && S.step === 1) {
        delete global.__renewtr_sessions[key];
  ...
  async function cont(bot, msg) {
    ...
    if (S.step === 2) {
      ...
      // validasi username
      const check = validateTrojanUser(S.user);
      if(!check.ok){
    ...
    if (S.step === 3) {
      ...
      try{
        logPurchase({
          tg_id : msg.from.id,
          kind  : (title||'renew-trojan').toLowerCase().replace(/\s+/g,'-'),
          days  : days,
