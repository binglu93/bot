...
// === callback tombol pilih server ===
async function handleVpsPick(bot, query) {
  const data = query.data;
  if (!data.startsWith('pickvps:')) return false;

  const idx = parseInt(data.split(':')[1]);
  const key = Object.keys(global.__addssh_sessions || {}).find(k => k.includes(query.from.id));
  if (!key) {
    await bot.answerCallbackQuery(query.id, { text: 'Sesi sudah kadaluarsa.', show_alert: true });
    return true;
  }
  ...
  await bot.sendMessage(query.message.chat.id, '👤 Masukkan *username*:', { parse_mode: 'Markdown' });
  return true;
}
...
function createAddSshPlugin({ name, aliases=[], title, commandTpl, expMode='days', hargaPerHari=0 }) {
  ...
  async function cont(bot, msg){
    ...
    if (S.step === 4) {
      ...
      await send(bot,msg.chat.id,`✅ SSH berhasil dibuat !\n\n${res.combined||'(no output)'}`);
...
module.exports = { createAddSshPlugin, handleVpsPick };
