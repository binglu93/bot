/// lib/addBaseWS.js
// Flow: pilih VPS -> username -> hari -> tampilkan list harga -> hitung harga -> cek & potong saldo (SQLite) -> SSH: printf "%s\n" "{USER}" "{EXP}" "2" "2" | addws
const fs = require('fs');
 ...
 *  4) tampilkan harga, cek saldo, potong saldo -> eksekusi SSH
 *
 * Options:
 *  - commandTpl: string, gunakan placeholder {USER} & {EXP}
 *  - expMode: 'days' | 'date'  (default 'days')
 *     - 'days' => EXP = String(hari)
 *     - 'date' => EXP = YYYY-MM-DD (hari ini + n)  (ubah kalau script butuh tanggal)
 */
  ...
  async function cont(bot, msg) {
    ...
    if (S.step === 3) {
      ...
      try {
        ...
        // contoh: "Add VMess" -> "add-vmess"
        const kind = (title || 'add-ws').toLowerCase().replace(/\s+/g, '-');
        logPurchase({
          tg_id : msg.from.id,
          kind  : kind,      // add-ssh | add-vmess | add-vless | add-trojan (sesuai title)
