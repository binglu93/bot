...
async function buildHeaderText(msg){
  ...
  const status = vpsList.length ? vpsList.map(s=>`${s.online?'🟢':'🔴'} ${escapeMd(s.name)}`).join('\n') : '_Tidak ada VPS terdaftar_';
...
async function showUserList(bot, q){
  try{
    const rows = db.prepare(`SELECT name,balance,created_at FROM users ORDER BY created_at DESC LIMIT 10`).all();
